...
pub fn from_json(json_str: &str, path: &str) -> Value {
    ...
    from_value_with_opts(&data, path, &JpOptions::default())
}
...
/// Internal engine
fn from_value_with_opts(data: &Value, path: &str, opts: &JpOptions) -> Value {
...
fn parse_expr(input: &str) -> Result<ENode, EParseErr> {
        ...
        ENode::Call { name, args } => {
            match name.as_str() {
                ...
                "from_json" => {
                    ...
                    let json_s = match eval_ast(&args[0]) {
                        Value::String(s) => s,
                        _ => return Value::Null,
                    };
                    let path_s = match eval_ast(&args[1]) {
                        Value::String(s) => s,
                        _ => return Value::Null,
                    };
                ...
                // or_default(expr, "<json default>")
                "or_default" => {
                    if args.len() != 2 {
                        return Value::Null;
                    }
                    let v = eval_ast(&args[0]);
                    let d = match eval_ast(&args[1]) {
                        Value::String(s) => s,
                        _ => return Value::Null,
                    };
                    or_default(&v, &d)
